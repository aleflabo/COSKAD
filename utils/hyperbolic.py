...
def poincare_dist(x, y, c=1.0, keepdim=False):
    r"""
    ...
    c = torch.as_tensor(c).type_as(x)
    sqrt_c = c ** 0.5
    dist_c = artanh(sqrt_c * mobius_add(-x, y, c).norm(dim=-1, p=2, keepdim=keepdim))
...
class FrechtMean(torch.nn.Module):
    ...
    def forward(self, X, w=None, K=-1.0, max_iter=1000, rtol=1e-6, atol=1e-6):
        ...
        if self.w is None:
            w = torch.ones(X.shape[0]).to(X.device) # corretto da 1 a 0 (batch_first)
        ...
        for _ in range(self.max_iter):
            ...
            alphas = self.l_prime(-self.K * xmu_ss / ((1 + self.K * x_ss + 1e-10) * # messo un epsilon al denominatore
                                  (1 + self.K * mu_ss.unsqueeze(-1)))) / (1 + self.K * x_ss + 1e-10) # messo un epsilon al denominatore
...
>>>>>>> cacebaf2220f8c728a5dd9badb94467ccd371864
        Returns
        -------
