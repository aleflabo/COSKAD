...
def windows_based_loss_hy(hidden_c, hidden_out_fig, frames_fig, n_frames, loss_fn=nn.MSELoss(reduction='none'), hyperbolic=False):
    
    w = hidden_out_fig.shape[0]
    
    if not hyperbolic:
        hidden_out_fig = torch.from_numpy(hidden_out_fig).cuda()
        loss = loss_fn(hidden_c,hidden_out_fig)
        loss = torch.mean(loss, dim=-1)
    else:
        curvature = torch.tensor(-1.)
        loss = gmath.dist(hidden_out_fig,hidden_c,k=curvature)
